...
public class CrazyGenerics {
     ...
     * @param <T> – value type
     */
    public static class MaxHolder<T> { // todo: refactor class to make it generic
        private Comparable<T> max;

        public MaxHolder(Comparable<T> max) {
            this.max = max;
        }
...
//            throw new ExerciseNotCompletedException(); // todo: add method parameters and implement the logic
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(List<T> entities,
                                                                   T targetEntity) {
            long count = entities.stream().filter(e -> e.getUuid().equals(targetEntity.getUuid())).count();
            return count > 1;
